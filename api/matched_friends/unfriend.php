...
if(isset($_GET['id'])){
    ...
    try{
        if($matchedFriend->cancelRequest()){
            // tell the user
            echo json_encode(array("succ" => true, "msg" => "Unfriended successfully!"));
        }
    }catch(Exception $e){
        echo json_encode(array("succ" => false, "msg" => "Unable to unfriend. Something went wrong!", "error" => $e->getMessage()));
    }

}else{
    // tell the user incomplete data
    echo json_encode(array("succ" => false, "msg" => "Unable to unfriend. Incomplete Data"));
}
?>